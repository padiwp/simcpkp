import 'dart:async';
import 'package:app/pages/result_pretest.dart';
...
class QuizPreTest extends StatefulWidget {
  const QuizPreTest({
    super.key,
  ...
  @override
  State<QuizPreTest> createState() => _QuizPreTestState();
}

class _QuizPreTestState extends State<QuizPreTest> {
  ...
  void initState() {
    ...
    _resetValues();
    _countdown =
        60 * int.parse(widget.testDuration); // Convert duration to seconds
    ...
    _countdownTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (_countdown > 0) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => ResultPreTestPage(
        ...
        timer.cancel();
        // Activate submit button when time's up
        setState(() {
      ...
      child: Scaffold(
        appBar: AppBar(
          automaticallyImplyLeading:
              false, // Set automaticallyImplyLeading to false
          backgroundColor: Colors.white,
          title: const Text("Ujian Pre Test"),
          actions: [
            StreamBuilder<int>(
              ...
              return Column(
                children: [
                  ScrollableCard(
                    ...
                    onCardTap: _handleCardTap, // Pass the callback function
                  ),
                  const SizedBox(
                    height: 25,
                    child: Text(
...
class ScrollableCard extends StatefulWidget {
  ...
  final Function(int) onCardTap; // New parameter for handling card tap
        ...
        children: List.generate(
          ...
          (index) => Padding(
            ...
            child: GestureDetector(
              // Wrap with GestureDetector for handling taps
              onTap: () {
                widget.onCardTap(index); // Call the callback on tap
              },
              child: Container(
                width: 50,
                height: 50,
